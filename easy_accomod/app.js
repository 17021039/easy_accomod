...
function authenticateToken(req,res,next) {
    try {
        if(req.query)
            ...
            res.set({
                'Access-Control-Allow-Origin': process.env['Access-Control-Allow-Origin']
            });
        ...
        if(!authenticationToken) {
            ...
            if((req.method === 'GET' || req.method === 'OPTIONS') && notGetUrl === -1)
                next_ = true;
...
function error_server(error, req, res, next) {
    if(error.message && error.message.toLowerCase().indexOf('đăng nhập') !== -1)
        error.status = 401;
    console.log(error);
    if(error === true)
        res.send(true);
