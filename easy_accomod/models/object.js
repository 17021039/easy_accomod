...
module.exports.renter = (renterId_, userId_, code_, name_, phone_, email_, roomFollow_) => {
    let renter = {}
    if(renterId_)
...
// xuất ra object notices
module.exports.notices = (noticesId_, ownerId_, notification_, readed_) => {
    let notices = {};
    if(readed_)
        readed_ = convertBoolean(readed_,'readed');

    if(noticesId_)
        notices.noticesId = noticesId_;
    if(ownerId_)
        notices.ownerId = ownerId_;
    if(notification_)
        notices.notification = notification_;
    if(readed_ === true || readed_ === false)
        notices.readed = readed_;
    return notices;
}
...
module.exports.containKeys = (nameObj, listKeys = [], required = false, get = false) => {
    let keys = {
        ...
        renter: ['renterId', 'userId', 'code', 'name', 'phone', 'email', 'roomFollow'],
        comment: ['commentId', 'renterId', 'roomId', 'comment', 'star'],
        notices: ['noticesId', 'ownerId', 'notification', 'readed'],
